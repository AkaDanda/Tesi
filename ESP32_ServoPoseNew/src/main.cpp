...
//Release arm variable
const int deploy_angle = 90;
const int post_angle = 0;
...
void keyboard_input_callback(const void * msgin) {
  ...
  if (deployment_counter == 1 ){
  servo1.write(deploy_angle);
  delay(5000);
  servo1.write(post_angle);
  }

  else if (deployment_counter == 2 ){
  servo2.write(deploy_angle);
  delay(5000);
  servo2.write(post_angle);
  }

  else if (deployment_counter == 3 ){
  servo3.write(deploy_angle);
  delay(5000);
  servo3.write(post_angle);
  }

  servoA.write(close_angle);
